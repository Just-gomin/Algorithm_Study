...
const solution = (new_id = "") => {
  ...
  // 1단계 new_id의 모든 대문자를 대응되는 소문자로 치환합니다.
  // 2단계 new_id에서 알파벳 소문자, 숫자, 빼기(-), 밑줄(_), 마침표(.)를 제외한 모든 문자를 제거합니다.
  let copyID = new_id.toLowerCase().match(/[a-z]|[0-9]|[.\-_]|\s/g);
  console.log(copyID);

  copyID.map((value, index) => {
    // 3단계 new_id에서 마침표(.)가 2번 이상 연속된 부분을 하나의 마침표(.)로 치환합니다.
    if (value === ".") {
      if (answer[answer.length - 1] !== ".") answer.push(value);
    } else {
      answer.push(value);
    }
  });
  console.log("first modify :", answer.join(""));
  // 4단계 new_id에서 마침표(.)가 처음이나 끝에 위치한다면 제거합니다.
  if (answer[0] === ".") answer.splice(0, 1);
  if (answer[answer.length - 1] === ".") answer.splice(answer.length - 1, 1);
  console.log("second modify :", answer.join(""));
  if (answer.join("") === "") {
    copyID = ["a"];
  }
  // 6단계 new_id의 길이가 16자 이상이면, new_id의 첫 15개의 문자를 제외한 나머지 문자들을 모두 제거합니다.
  ...
  if (answer.length > 15) {
    ...
    if (answer[14] === ".") answer.splice(14, 1);
    console.log("after slice : ", answer.join(""));
  }
  // 7단계 new_id의 길이가 2자 이하라면, new_id의 마지막 문자를 new_id의 길이가 3이 될 때까지 반복해서 끝에 붙입니다.
  if (answer.length < 3) {
    let lastIndex = copyID.length - 1;
    while (answer.length < 3) {
      if (copyID[lastIndex] === ".") {
        copyID.splice(lastIndex, 1);
        lastIndex = copyID.length - 1;
      }
      answer.push(copyID[lastIndex]);
      if (answer[0] === ".") answer.splice(0, 1);
      if (answer[2] === ".") answer.splice(2, 1);
    }
  }
  return answer.join("") === "" ? "aaa" : answer.join("");
};
